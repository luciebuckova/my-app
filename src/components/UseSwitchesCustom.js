...
export default function UseSwitchesCustom(props) {
  ...
  return <MUISwitch toggleMode={toggleMode} {...props} />;
}

function MUISwitch({ toggleMode, ...props }) {
  ...
  const handleToggle = () => {
    if (toggleMode && typeof toggleMode === "function") {
      toggleMode();
    }
  };

  return (
    <SwitchRoot className={clsx(stateClasses)} onClick={handleToggle}>
      <SwitchTrack>
        <SwitchThumb className={clsx(stateClasses)} />
      </SwitchTrack>
      <SwitchInput {...getInputProps()} aria-label="Dark mode switch" />
    </SwitchRoot>
  );
}
...
const SwitchTrack = styled("span")(
  ({ theme }) => `
  background-color: ${theme.palette.mode === "dark" ? teal[800] : teal[400]};
  border-radius: 4px;
  width: 100%;
  height: 100%;
  display: block;
